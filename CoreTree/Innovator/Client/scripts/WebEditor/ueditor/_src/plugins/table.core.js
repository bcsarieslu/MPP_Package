...
(function () {
    ...
    UETable.prototype = {
         ...
         * 插入一行单元格
         */
        insertRow:function (rowIndex, sourceCell,part) {
			//Modify by tengz
			//处理有合并单元格的情况时读取Part的属性信息
			var partProIndex=0;
            ...
            if (rowIndex == 0 || rowIndex == this.rowsNum) {
                for (var colIndex = 0; colIndex < numCols; colIndex++) {
					...
					if(part)
					{
						if(UE.itemTypesPros[part.getAttribute("type")][partProIndex])
						{
							cell.textContent=top.aras.getItemProperty(part,UE.itemTypesPros[part.getAttribute("type")][partProIndex],"");
							cell.setAttribute("name",UE.itemTypesPros[part.getAttribute("type")][partProIndex]);
							cell.setAttribute("contenteditable","false");
							cell.setAttribute("class","bcsTableItemClass");
						}
						else
						{
							//因为Cell是复制的,所以这个时候要把Cell的属性清掉
			...
			//Modify by tengz
			//为Part行添加参数以便编辑器加载内容时对Part行进行特殊处理(也就是当Part属性变更后编辑器内会显示更新后的属性)
			if(part)
			{
				row.id=part.getAttribute("id");
				row.setAttribute("name","bcsItem");
				row.setAttribute("itemtype",part.getAttribute("type"))
			}
