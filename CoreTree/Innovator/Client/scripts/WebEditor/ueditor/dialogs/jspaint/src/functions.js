...
function file_save(){
	...
	var base64=canvas.toDataURL("image/png").substring(22);
	if(!!base64)
	{
		var result=top.aras.IomInnovator.applyMethod("bcs_mpp_uploadGraffitiImage","<imgbase>"+base64+"</imgbase>");
		if(result.isError())
		{
		...
		var imgObj = {},
			url = top.aras.IomInnovator.getFileUrl(result.getID(), top.aras.Enums.UrlType.SecurityToken);
		imgObj.src = url;
		imgObj._src = url;
		imgObj.name="bcsItem";
		imgObj.itemtype="File";
		imgObj.id=result.getID();
		// imgObj.alt = responseObj.original || '';
		// imgObj.title = responseObj.title || '';
		dialog.editor.execCommand("insertImage", imgObj);
		dialog.close();
