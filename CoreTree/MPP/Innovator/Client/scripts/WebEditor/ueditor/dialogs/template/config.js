...
//读取模板
var templates=[];
var bcs_sop_teamplates=top.aras.IomInnovator.newItem("BCS SOP Template","get");
bcs_sop_teamplates.setProperty("classification","模板");
bcs_sop_teamplates.setAttribute("select","name,bcs_details,classification,bcs_head,bcs_footer");
bcs_sop_teamplates=bcs_sop_teamplates.apply();
for(var i=0;i<bcs_sop_teamplates.getItemCount();i++)
{
	var template=bcs_sop_teamplates.getItemByIndex(i);
	var title=template.getProperty("name");
	var html="";
	var is_headfooter=false;
	var headHtml="";
	var footerHtml="";
	
	switch(template.getProperty("classification",""))
	{
		case "模板":
			html=template.getProperty("bcs_details","");
			break;
		case "页头页尾":
			//此处已作废
			headHtml=template.getProperty("bcs_head","");
			footerHtml=template.getProperty("bcs_footer","");
			html=headHtml+footerHtml;
			is_headfooter=true;
			break;
	}
	
	templates.push({'pre':"pre4.png",'title':title,'preHtml':html,'html':html,'is_headfooter':is_headfooter,'headHtml':headHtml,'footerHtml':footerHtml});
}