(function (b) {
    ...
    a.rect = function (p, m) {
        ...
        b([t.node, f.node, n.node, e.node]).bind("mousedown", function (y) {
            if (!EditMode.isEditMode) {
                return;
            }
            if (!a.config.editable) {
            ...
            if (y.button != 2) {
                b("#contextmenu").css({ "display": "none" });
                return false;
            }
            w();
            var o = b(C).data("mod");
            ...
            b(C).data("currNode", u);
            rightClick = u;
            b("#contextmenu").css({ "top": y.offsetY + "px", "left": y.offsetX + "px", "display": "block" });
            return false
        });
        ...
        function B() {
            ...
            var drag = function (ev) {
                var conY = false,
                    ...
                    actualX = ev.x + 60,
                    actualY = ev.y + 60,
                    bodywidth = $("#myflow>svg").css("width"),
                    bodyheight = $("#myflow>svg").css("height");
                bodywidth = +(bodywidth.substring(0, bodywidth.length - 2));
                bodyheight = +(bodyheight.substring(0, bodyheight.length - 2));
                if (bodyheight > +actualY && bodywidth > +actualX) {
                    bodyActualMaxY[g] = actualY;
                    bodyActualMaxX[g] = actualX;
                }
                else if (bodyheight < actualY && bodywidth < +actualX) {
                    bodyActualMaxY[g] = actualY;
                    bodyActualMaxX[g] = actualX;
                }
                else if (bodyheight > actualY && bodywidth < +actualX) {
                ...
                for (k in bodyActualMaxX) {
                    if (bodyActualMaxX[k] > actualX) {
                        conX = true;
                        actualX = document.documentElement.scrollLeft;
                        break;
                    }
                }
                for (k in bodyActualMaxY) {
                    if (bodyActualMaxY[k] > actualY) {
                        conY = true;
                        actualY = document.documentElement.scrollTop;
                        break;
                    }
                }
                if (!conX) {
                    if ((+actualX) > +bodywidth) {
                        $("#myflow>svg").css("width", +actualX + "px");
                    }
                    else {
                        if (actualX < parent.document.getElementById("formContentPane").offsetWidth - 5) {
                            $("#myflow>svg").css("width", parent.document.getElementById("formContentPane").offsetWidth - 15 + "px");
                        }
                        else {
                            $("#myflow>svg").css("width", +actualX + "px")
                        }
                ...
                if (!conY) {
                    if ((+actualY) > +bodyheight) {
                        $("#myflow>svg").css("height", +actualY + "px");
                    }
                    else {
                        if (actualY < parent.document.getElementById("formContentPane").offsetHeight - 5) {
                            $("#myflow>svg").css("height", parent.document.getElementById("formContentPane").offsetHeight - 20 + "px");
                        }
                        else {
                            $("#myflow>svg").css("height", +actualY + "px");
                        }
                ...
                window.scrollTo(actualX, actualY);
            };
            drag({
                x: F,
                y: r,
    ...
    a.path = function (q, n, u, e) {
            ...
            this.moveTo = function (Q, T) {
                ...
                switch (M) {
                    ...
                    case "big":
                        ...
                        if (q.id) {
                            courseAmend(x.toJson(), q.id);
                        }
                        else {
                            courseAmend(x.toJson(), courseID);
                        }
                        break;
                    case "small":
        ...
        b([i.node, t.node, f.node]).bind("mousedown", function (e) {
            if (!a.config.editable) {
                return;
            }
            if (!EditMode.isEditMode) {
            ...
            b(z).data("currNode", v);
            rightClick = v;
            b("#contextmenu").css({ "top": e.offsetY + "px", "left": e.offsetX + "px", "display": "block" });
            return false
        });
        var l = function (r, C) {
            if (!a.config.editable) {
    ...
    a.init = function (x, r) {
        ...
        b(document).click(bodyClick);
        var w = function (c, i) {
            if (!a.config.editable) {
                return
            }
            if (i.getId().substring(0, 4) == "rect") {
                removeparocess(i.getId());
                q[i.getId()] = null;
                i.remove();
                delete q[i.getId()];
                bodyActualMaxX[i.getId()] = null;
                bodyActualMaxY[i.getId()] = null;
                delete bodyActualMaxX[i.getId()];
                delete bodyActualMaxY[i.getId()];
            } else {
